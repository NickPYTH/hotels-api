...
public class FlatController {
    ...
    public ResponseEntity<List<FlatDTO>> getAllByHotelId(@RequestParam Long id, @RequestParam String date) {
    ...
    public ResponseEntity<List<HashMap<String, String>>> getAllByHotelIdChess(@RequestParam Long id, @RequestParam String dateStart, @RequestParam String dateFinish) {
        long startTime = System.nanoTime();
        Log record = new Log();
        try {
    ...
    public ResponseEntity<FlatDTO> updateStatus(@RequestParam Long id) {
        ...
        try {
            ...
            record.setStatus("OK");
            record.setUser(SecurityManager.getCurrentUser());
            record.setPath("/flat/updateStatus");
            record.setDuration(duration);
            record.setDate(new Date());
            logsRepository.save(record);
            return new ResponseEntity<>(response, HttpStatus.OK);
            ...
            Double duration = (System.nanoTime() - startTime) / 1E9;
            logger.info(loggerString, dateTimeFormatter.format(new Date()), "ERROR", SecurityManager.getCurrentUser(), "/flat/updateStatus", duration, e.getMessage());
            record.setStatus("ERROR");
            record.setUser(SecurityManager.getCurrentUser());
            record.setPath("/flat/updateStatus");
            record.setDuration(duration);
